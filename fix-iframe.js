...
for (const file of files) {
  ...
  // Check if this file has a fixlify booking iframe
  if (!content.includes('hub.fixlify.app/book/nicks-appliance-repair-b8c8ce')) continue;
  
  // Already has the new id and resize script? Skip.
  if (content.includes('fixlify-booking-nicks-appliance-repair-b8c8ce') && content.includes('fixlify-resize')) continue;
  
  const original = content;
  
  // Replace any iframe with fixlify booking URL (capture entire iframe tag with all variations)
  // Pattern: <iframe ... src="https://hub.fixlify.app/book/nicks-appliance-repair-b8c8ce..."> ... </iframe>
  // followed optionally by a resize script block
  
  // Remove old resize script if present (near the iframe)
  ...
  // Replace old iframe tag (single line or multiline)
