#!/usr/bin/env python3
"""
BMAD Blog Post Generator - Unique Content Version
Generates SEO-optimized blog posts using unique content library.
Each post has 95%+ unique content per BMAD V2 Param 16 requirement.
...
# Import unique content library
from unique_content_library import UNIQUE_POSTS

# 20 Blog Posts metadata based on DataForSEO Keyword Gap Analysis
BLOG_POSTS = [
    {
...
def generate_faq_html(faq_list):
    """Generate FAQ HTML from list of Q&A pairs"""
    faq_html = """
...
def generate_blog_post(post_meta, unique_content, publish_date):
    """Generate complete blog post HTML using unique content"""

    brand = post_meta.get("brand", "")
    brand_text = f"{brand} " if brand else ""
    title = unique_content["title"]

    # Generate FAQ HTML from list
    faq_html = generate_faq_html(unique_content["faq"])
...
<head>
    ...
    <meta name="description" content="Expert {post_meta["keyword"]} repair in Toronto. Professional troubleshooting guide with step-by-step solutions. Same-day service available. Call (437) 747-6737.">
    <meta name="keywords" content="{post_meta["keyword"]}, toronto {post_meta["appliance"]} repair, {post_meta["appliance"]} troubleshooting, appliance repair toronto">
    ...
    <link rel="canonical" href="https://nikaappliancerepair.com/blog/{post_meta["category"]}/{post_meta["slug"]}">
    <meta property="og:title" content="{title}">
    <meta property="og:description" content="Expert {post_meta["keyword"]} repair guide for Toronto homeowners">
    <meta property="og:type" content="article">
    <meta property="og:url" content="https://nikaappliancerepair.com/blog/{post_meta["category"]}/{post_meta["slug"]}">
...
{{
  ...
  "mainEntityOfPage": {{
    "@type": "WebPage",
    "@id": "https://nikaappliancerepair.com/blog/{post_meta["category"]}/{post_meta["slug"]}"
  }}
}}
    </script>
    <script type="application/ld+json">
{{
  ...
  "itemListElement": [
    ...
    {{"@type": "ListItem", "position": 3, "name": "{post_meta["category"].title()}", "item": "https://nikaappliancerepair.com/blog/{post_meta["category"]}"}}
  ]
}}
    </script>
    <!-- Footer Styles -->
        ...
        <main class="blog-main">
            <header class="blog-header">
                <div class="blog-meta-top">
                    <span class="blog-category"><i class="fas fa-wrench"></i> {post_meta["category"].title()}</span>
            ...
            <article class="blog-content">
                {unique_content["intro"]}

                {unique_content["problems"]}

                {unique_content["diy"]}

                <div class="cta-box">
                    <h3>Need Professional Help with {brand_text}{post_meta["appliance"].title()} Repair?</h3>
                ...
                {unique_content["costs"]}

                <h2 id="when-to-call">When to Call a Professional</h2>

                <p>While some {post_meta["appliance"]} issues can be resolved with DIY troubleshooting, certain situations require professional expertise:</p>
                ...
                <p>We provide expert {brand_text}{post_meta["appliance"]} repair throughout the Greater Toronto Area:</p>
                ...
                <h2 id="conclusion">Get Your {brand_text}{post_meta["appliance"].title()} Fixed Today</h2>

                <p>Don't let {post_meta["keyword"]} problems disrupt your daily routine. Whether you need a quick diagnosis or a complete repair, Nika Appliance Repair is here to help. Our experienced technicians provide honest assessments, transparent pricing, and quality repairs backed by our 90-day warranty.</p>

                <div class="cta-box">
                    <h3>Schedule Your Repair Today</h3>
                    <p>Same-day service available. Call now for expert {brand_text}{post_meta["appliance"]} repair!</p>
                    <a href="tel:4377476737" class="btn">
                        <i class="fas fa-phone"></i> Call (437) 747-6737
                    </a>
                </div>
...
def main():
    """Generate all 20 blog posts with unique content"""
    ...
    print("=" * 70)
    print("BMAD BLOG POST GENERATOR - UNIQUE CONTENT VERSION")
    print("Generating 20 SEO-optimized posts with 95%+ unique content")
    print("BMAD V2 Param 16: Content Uniqueness - COMPLIANT")
    print("=" * 70)
    print(f"\nOutput: {drafts_dir}")
    print(f"Posts: {len(BLOG_POSTS)}")
    print(f"Unique content entries: {len(UNIQUE_POSTS)}\n")
    ...
    for i, post_meta in enumerate(BLOG_POSTS):
        # Each post gets published one day apart
        publish_date = (start_date + timedelta(days=i)).strftime('%Y-%m-%d')
        slug = post_meta['slug']

        # Check if unique content exists for this post
        if slug not in UNIQUE_POSTS:
            print(f"[SKIP] {slug} - no unique content found")
            skipped += 1
            continue

        unique_content = UNIQUE_POSTS[slug]

        filename = f"{slug}.html"
        filepath = drafts_dir / filename

        html_content = generate_blog_post(post_meta, unique_content, publish_date)

        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(html_content)

        print(f"[{i+1:02d}/{len(BLOG_POSTS)}] {filename}")
        print(f"         Title: {unique_content['title']}")
        print(f"         Keyword: {post_meta['keyword']} (vol: {post_meta['volume']})")
        print(f"         Publish: {publish_date}")
        generated += 1

    print(f"\n{'=' * 70}")
    print(f"[SUCCESS] Generated {generated} blog posts with unique content")
    if skipped > 0:
        print(f"[WARNING] Skipped {skipped} posts (no unique content)")
    ...
    with open(summary_path, 'w', encoding='utf-8') as f:
        ...
        f.write("BMAD V2 Param 16 Compliant - 95%+ Unique Content\n")
        f.write("=" * 50 + "\n\n")
        for i, post_meta in enumerate(BLOG_POSTS):
            if post_meta['slug'] in UNIQUE_POSTS:
                publish_date = (start_date + timedelta(days=i)).strftime('%Y-%m-%d')
                title = UNIQUE_POSTS[post_meta['slug']]['title']
                f.write(f"{publish_date}: {title}\n")
                f.write(f"           URL: /blog/{post_meta['category']}/{post_meta['slug']}\n")
                f.write(f"           Keyword: {post_meta['keyword']} ({post_meta['volume']} vol)\n\n")
